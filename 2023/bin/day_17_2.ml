...
let () =
  ...
  let costs, dirs, straight = Heat_Grid.distance grid src in
  ...
  Format.(
    ...
    Grid.pp ~highlight:path
      ...
      err_formatter dirs;
    pp_print_string err_formatter "==== Num Steps ====\n";
    Grid.pp ~highlight:path (fun fmt steps -> fprintf fmt " %3d" steps) err_formatter straight;
