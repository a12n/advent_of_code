package body Advent.Day_04 is
   ...
   function Has_Word (Letters : Word_Search;
                      Word : String;
                      Row, Col : Positive;
                      Direction : Direction_Type) return Boolean is
   begin
      ...
      if Row + Direction (1) not in Letters'Range (1) or
        Col + Direction (2) not in Letters'Range (2) then
         return False;
      end if;
      return Has_Word (Letters, Word (Word'First + 1 .. Word'Last),
        Row + Direction (1), Col + Direction (2), Direction);
   end Has_Word;

   function Num_Words (Letters : Word_Search;
                       Word : String;
                       Row, Col : Positive) return Natural is
   begin
      ...
      for Vert in -1 .. 1 loop
         for Horiz in -1 .. 1 loop
            if Vert/=0 or Horiz /= 0 then
               if Has_Word (Letters, Word, Row, Col, Direction_Type'(Vert, Horiz)) then
                  N := N + 1;
               end if;
            end if;
         end loop;
